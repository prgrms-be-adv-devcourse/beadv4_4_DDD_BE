...
public class OAuthAccountBindingUseCase {
  ...
  public void link(Long memberId, OAuthProvider provider, OAuthUserInfo userInfo) {
    ...
    boolean alreadyUsedByOther = socialAccountRepository
        .existsByOauthProviderAndProviderAccountId(provider, userInfo.getProviderId());
    ...
    boolean alreadyLinked = socialAccountRepository
        .existsByMemberIdAndOauthProvider(memberId, provider);
    ...
    // 4. 소셜 계정 엔티티 생성 및 연동
    log.info("기존 회원에 소셜 계정 연동 - memberId: {}, provider: {}, providerId: {}",
        member.getId(), userInfo.getProvider(), userInfo.getProviderId());

    OAuthAccount socialAccount = OAuthAccount.builder()
    ...
    log.info("소셜 계정 연동 완료 - memberId: {}, provider: {}, providerId: {}",
        memberId, provider, socialAccount.getProviderAccountId());
  }
}